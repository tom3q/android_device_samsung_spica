...
void mixer_close(struct mixer *mixer)
{
	...
	if (mixer->ctl) {
		for (n = 0; n < mixer->count; n++) {
			...
			max = mixer->ctl[n].info->value.enumerated.items;
...
static int mixer_ctl_compare(const void *a, const void *b)
{
	struct mixer_ctl *ctl_a = *(struct mixer_ctl **)a;
	struct mixer_ctl *ctl_b = *(struct mixer_ctl **)b;
	int val;

	val = ctl_a->info->id.index - ctl_b->info->id.index;
	if (val)
		return val;

	return strcmp((const char *)ctl_a->info->id.name,
					(const char *)ctl_b->info->id.name);
}
...
struct mixer *mixer_open(void) {
	...
	mixer->lookup = calloc(elist.count, sizeof(struct mixer_ctl *));

	if (!mixer->ctl || !mixer->info || !mixer->lookup)
		goto fail;
	...
	for (n = 0; n < mixer->count; n++) {
		...
		mixer->lookup[n] = &mixer->ctl[n];

		if (ei->type != SNDRV_CTL_ELEM_TYPE_ENUMERATED)
			continue;

		char **enames = calloc(ei->value.enumerated.items,
							sizeof(char*));
		if (!enames)
			goto fail;
	...
	qsort(mixer->lookup, mixer->count,
				sizeof(*mixer->lookup), mixer_ctl_compare);
					...
					const char *name, unsigned index)
{
	struct snd_ctl_elem_info info;
	struct mixer_ctl ctl;
	struct mixer_ctl *key = &ctl;
	struct mixer_ctl **ret;

	strncpy((char *)info.id.name, name, sizeof(info.id.name));
	info.id.index = index;

	ctl.info = &info;

	ret = bsearch(&key, mixer->lookup, mixer->count,
				sizeof(*mixer->lookup), mixer_ctl_compare);
...
void mixer_ctl_print(struct mixer_ctl *ctl)
{
	...
	case SNDRV_CTL_ELEM_TYPE_ENUMERATED:
		for (n = 0; n < ctl->info->count; n++) {
			...
			printf(" %d (%s)", v,
			       (v < ctl->info->value.enumerated.items) ?
							ctl->ename[v] : "???");
		}
...
int mixer_ctl_select(struct mixer_ctl *ctl, const char *value)
{
	...
	for (n = 0; n < max; n++) {
		if (!strcmp(value, ctl->ename[n])) {
			...
			if (ioctl(ctl->mixer->fd,
			    SNDRV_CTL_IOCTL_ELEM_WRITE, &ev) < 0)
				return -1;
